...
function rotateMatrix(matrix) {
  ...
  for (let i = 0; i < newMatrix.length; i += 1) {
    for (let j = 0; j < newMatrix[0].length; j += 1) {
      result[j][newMatrix.length - i - 1] = newMatrix[i][j];
    }
  }
...
function sortByAsc(arr) {
  ...
  for (let i = 1; i < sortArr.length; i += 1) {
    if (arr[i] <= pivot) {
      left[indexStart] = sortArr[i];
      indexStart += 1;
    } else {
      right[indexEnd] = sortArr[i];
      indexEnd += 1;
    }
  }

  result = [...sortByAsc(left), pivot, ...sortByAsc(right)];
